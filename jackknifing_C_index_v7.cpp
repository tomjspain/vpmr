...
// [[Rcpp::export]]
double TJS_CIndex(CharacterVector ids,
                  NumericVector obs,
                  NumericVector pred,
                  std::string type) {
  int n = ids.size();
...
// [[Rcpp::export]]
NumericVector jk_TJS_CIndex(CharacterVector ids,
                            NumericVector obs,
                            NumericVector pred,
                            std::string type) {
  int n = ids.size();
...
// [[Rcpp::export]]
Rcpp::NumericVector gjk_partition_random(CharacterVector ids,
                                         NumericVector obs,
                                         NumericVector pred,
                                         std::string type,
                                         int d,
  ...
  double theta_full = TJS_CIndex(ids, obs, pred, type);
  ...
  for (int g = 0; g < G; ++g) {
    ...
    // Safety: ensure at least 2 remain (TJS_CIndex requires >= 2)
    if (ids_keep.size() < 2) {
    ...
    reps[g] = TJS_CIndex(ids_k, obs_k, pred_k, type);
  }
  ...
  reps.attr("method")       = "grouped_jackknife_partition_random";

  return reps;
}

// [[Rcpp::export]]
Rcpp::NumericVector gjk_random_overlapping(CharacterVector ids,
                                           NumericVector obs,
                                           NumericVector pred,
                                           std::string type,
                                           int d,
  ...
  // Validate index type (keep consistent with TJS_CIndex)
  std::vector<std::string> valid = {
    "C type1", "C type2", "C type3",
    "kendall", "goodman", "C type4", "somer"
  };
  ...
  for (int r = 0; r < n_reps; ++r) {
    ...
    reps[r] = TJS_CIndex(ids_k, obs_k, pred_k, type);
  }
  ...
  reps.attr("d_is_percent") = d_is_percent;
  reps.attr("method")       = "grouped_jackknife_random_overlapping";
...
// [[Rcpp::export]]
Rcpp::NumericVector gjk_random_overlapping_fast_with_sets(CharacterVector ids,
                                                          NumericVector obs,
                                                          NumericVector pred,
                                                          std::string type,
  ...
  reps.attr("method")       = "grouped_jackknife_random_overlapping_fast_perid_sets";
